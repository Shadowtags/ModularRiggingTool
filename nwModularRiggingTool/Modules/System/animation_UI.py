...
class Animation_UI:
	...
	def __init__(self):
		...
		self.prebiousAnimationModule = None
		...
		self.UIElements["animationModule_textScroll"] = pm.textScrollList(numberOfRows = 12, allowMultiSelection = False, selectCommand = self.RefreshAnimationModuleList, parent = self.UIElements["listboxRowLayout"])
		
		self.UIElements["buttonColumnLayout"] = pm.columnLayout(parent = self.UIElements["listboxRowLayout"])
		self.UIElements["pinButton"] = pm.symbolCheckBox(onImage = "%s_pinned.xpm" %baseIconsDir, offImage = "%s_unpinned.xpm" %baseIconsDir, width = buttonWidth, height = buttonWidth, parent = self.UIElements["buttonColumnLayout"])
		...
		if pm.objExists("%s:non_blueprint_grp" %self.selectedCharacter):
			...
			self.UIElements["non_blueprintVisibility"] = pm.symbolCheckBox(image = "%s_shelf_character.xpm" %baseIconsDir, value = value, width = buttonWidth, height = buttonWidth, parent = self.UIElements["buttonColumnLayout"])
		
		value = pm.getAttr("%s:character_grp.animationControlVisibility" %self.selectedCharacter)
		self.UIElements["animControlVisibility"] = pm.symbolCheckBox(image = "%s_visibility.xpm" %baseIconsDir, value = value, width = buttonWidth, height = buttonWidth, parent = self.UIElements["buttonColumnLayout"])
